...
void print_page(const std::vector<Record>& page) {
    for (const auto& record : page) {
        ...
        std::cout << std::string_view(record.data())
                         .substr(0, record.data().find('\0'))
                  << std::endl;
    }
}
...
std::vector<Record> create_page_data(int page_num, int num_records) {
    ...
    for (int i = 0; i < num_records; ++i) {
        page_data.push_back(Record(
            "Page " + std::to_string(page_num) + ", Record " + std::to_string(i)
        ));
    }
    return page_data;
}
...
bool compare_pages(
    const std::vector<Record>& page1, const std::vector<Record>& page2
) {
    if (page1.size() != page2.size()) {
        std::cerr << "Page size mismatch: " << page1.size() << " vs "
                  << page2.size() << std::endl;
        return false;
    }
...
int main() {
    ...
    std::cout << "This test checks that for a newly created file, the begin() "
                 "and end() iterators are the same, and the page count is 0."
              << std::endl;
    {
        ...
        if (pages.begin() != pages.end()) {
            std::cerr << "Test 1 Failed: For empty file, begin() should be "
                         "equal to end()"
                      << std::endl;
            return 1;
        }
        if (bf.getPageCount() != 0) {
            std::cerr << "Test 1 Failed: Empty file should have 0 pages"
                      << std::endl;
            return 1;
        }
    }
    ...
    const int num_pages_to_test = 3;
    {
        ...
        auto pages = bf.pages();
        // Write
        std::cout << "Writing " << num_pages_to_test << " pages of data..."
                  << std::endl;
        ...
        if (bf.getPageCount() != num_pages_to_test) {
            std::cerr << "Test 2 Failed: Page count should be "
                      << num_pages_to_test << " but is " << bf.getPageCount()
                      << std::endl;
            return 1;
        }

        // Verify content with forward iteration
        std::cout << "Iterating forwards (begin() to end()) to read and verify "
                     "all pages..."
                  << std::endl;
        int page_idx = 0;
        for (auto it = pages.begin(); it != pages.end(); ++it, ++page_idx) {
            ...
            if (!compare_pages(read_data, expected_data)) {
                std::cerr << "Test 2 Failed: Content mismatch on page "
                          << page_idx << std::endl;
                return 1;
            }
        }
    ...
    std::cout << "Iterating backwards (from end() - 1) to read and verify all "
                 "pages..."
              << std::endl;
    {
        BufferedFile bf(filename);
        ...
        for (auto it = pages.end() - 1; it != pages.begin() - 1;
             --it, --page_idx) {
            std::vector<Record> read_data = *it;
            auto expected_data = create_page_data(page_idx, 5);
            expected_data.resize(BufferedFile::recordsPerPage, Record::empty);
            ...
            if (!compare_pages(read_data, expected_data)) {
                std::cerr << "Test 3 Failed: Content mismatch on page "
                          << page_idx << std::endl;
                return 1;
            }
        }
    ...
    std::cout << "\n--- Test 4: Random Access and Overwrite ---" << std::endl;
    {
        ...
        pages.begin()[1] = new_page1_data;
        bf.flush();

        std::cout << "Verifying overwritten page and checking that other pages "
                     "are unchanged..."
                  << std::endl;
        ...
        if (!compare_pages(*(pages.begin()), page0_data)) {
            std::cerr << "Test 4 Failed: Page 0 was modified unexpectedly"
                      << std::endl;
            return 1;
        }
        ...
        if (!compare_pages(pages.begin()[1], new_page1_data)) {
            std::cerr << "Test 4 Failed: Page 1 was not overwritten correctly"
                      << std::endl;
            return 1;
        }
        ...
        if (!compare_pages(*(pages.begin() + 2), page2_data)) {
            std::cerr << "Test 4 Failed: Page 2 was modified unexpectedly"
                      << std::endl;
            return 1;
        }

        std::cout << "Verifying iterator subtraction (end() - begin())..."
                  << std::endl;
        if ((pages.end() - pages.begin()) != num_pages_to_test) {
            std::cerr << "Test 4 Failed: Iterator difference is incorrect."
                      << std::endl;
            return 1;
        }
    }
    ...
    std::cout << "\n--- Test 5: Out of Bounds Access ---" << std::endl;
    {
        ...
        std::cout << "Trying to read past the end of the file, expecting an "
                     "exception..."
                  << std::endl;
        try {
            auto page_proxy = pages.begin()[bf.getPageCount()];
            page_proxy.records();
        } catch (const std::out_of_range& e) {
            exception_thrown = true;
        }
        if (!exception_thrown) {
            std::cerr << "Test 5 Failed: Did not throw for out of bounds read"
                      << std::endl;
            return 1;
        }

        std::cout << "Trying to write far past the end of the file, expecting "
                     "an exception..."
                  << std::endl;
        exception_thrown = false;
        try {
            pages.begin()[bf.getPageCount() + 5] = create_page_data(0, 1);
        } catch (const std::out_of_range& e) {
            exception_thrown = true;
        }
        if (!exception_thrown) {
            std::cerr << "Test 5 Failed: Did not throw for out of bounds write"
                      << std::endl;
            return 1;
        }
    }
    ...
    std::cout
        << "Writing to a new page at the end of the file to test append..."
        << std::endl;
    {
        BufferedFile bf(filename);
        auto pages = bf.pages();
        ...
        pages.begin()[initial_page_count] = append_data;
        bf.flush();

        if (bf.getPageCount() != initial_page_count + 1) {
            std::cerr
                << "Test 6 Failed: Page count did not increase after append."
                << std::endl;
            return 1;
        }

        append_data.resize(BufferedFile::recordsPerPage, Record::empty);
        if (!compare_pages(pages.begin()[initial_page_count], append_data)) {
            std::cerr << "Test 6 Failed: Appended page content is incorrect."
                      << std::endl;
            return 1;
        }
    }
